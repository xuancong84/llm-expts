...
GET_STATS_ERR = "CUDAPluggableAllocator does not yet support getDeviceStats"

def patch_rmm_pluggable_allocator_compat():
	# 1) Patch TorchInductor autotune path (torch.compile=True path)
	try:
		...
		def _patched(self, *args, **kwargs):
			try:
				...
				if GET_STATS_ERR in str(e):
					# Same behavior as newer upstream guard:
					# skip CPU-copy budget logic when stats are unavailable
					return {}
				raise

		th.CachingAutotuner.copy_args_to_cpu_if_needed = _patched
		print("[patch] TorchInductor patched for pluggable allocator stats fallback.")
	except Exception as e:
		print(f"[patch] TorchInductor patch skipped: {e}")

	# 2) Patch generic torch.cuda stat helpers (helps DS + other call sites)
	...
	torch.cuda.reset_max_memory_allocated = _safe_wrap(torch.cuda.reset_max_memory_allocated, None)

	# 3) Optional: patch DeepSpeed memory logger path
	try:
